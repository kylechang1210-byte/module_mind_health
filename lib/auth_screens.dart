    ...
    if (session != null) {
      ...
      try {
        final data = await Supabase.instance.client
            .from('user').select('role').eq('email', session.user.email!).maybeSingle();
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Welcome Back", textAlign: TextAlign.center, style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: kPrimaryBlue)),
            ...
            Align(
              ...
              child: TextButton(
                ...
                child: const Text("Forgot Password?", style: TextStyle(fontWeight: FontWeight.bold, color: kPrimaryBlue)),
              ),
            ),
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Future<void> _signUp() async {
    ...
    setState(() => _isLoading = true);
    try {
      final response = await Supabase.instance.client.auth.signUp(email: _emailCtrl.text.trim(), password: _passCtrl.text.trim());
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              const SizedBox(height: 20),
              TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: "Username", prefixIcon: Icon(Icons.badge))),
              const SizedBox(height: 16),
              TextField(controller: _emailCtrl, decoration: const InputDecoration(labelText: "Email", prefixIcon: Icon(Icons.email))),
              const SizedBox(height: 16),
              TextField(controller: _passCtrl, obscureText: true, decoration: const InputDecoration(labelText: "Password", prefixIcon: Icon(Icons.lock))),
              const SizedBox(height: 24),
              SizedBox(width: double.infinity, child: FilledButton(onPressed: _isLoading ? null : _signUp, child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text("Sign Up"))),
            ],
          ),
        ),
      ),
